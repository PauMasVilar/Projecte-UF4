...
@section('content')
    ...
    <h1 class="text-5xl font-bold text-center text-gray-800 m-10">Cursos online</h1>
                ...
                <div class="container mx-auto mt-4 p-4">
                    <div class="bg-white rounded-lg shadow-md p-6 h-full flex flex-col">
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="text-xl sm:text-2xl md:text-3xl font-semibold">{{ $course->name }}</h1>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div>
                                    @if (Auth::check() && (!Auth::user()->is_professor && !Auth::user()->is_admin))
                                        @if (!Auth::user()->enrollments->contains('id_course', $course->id))
                                            <a href="{{ route("studentEnrollment.create", ["course" => $course, "user" => Auth::user()]) }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out">
                                                Inscriu-te!
                                            </a>
                                        @else 
                                            <a href="{{ route("studentEnrollment.delete", ["course" => $course, "user" => Auth::user()]) }}" class="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition duration-300 ease-in-out">
